...
const App = () => {
  ...
  const customerOptions = useMemo(() => {
    const s = new Set();
    orders.forEach(o => { if (o.tenKhachHang) s.add(o.tenKhachHang); });
    return Array.from(s).map(v => ({ label: v, value: v }));
  }, [orders]);

  const bgOptions = useMemo(() => {
    const s = new Set();
    orders.forEach(o => { if (o.maDon) s.add(o.maDon); });
    return Array.from(s).map(v => ({ label: v, value: v }));
  }, [orders]);
  ...
  const orderCategorized = useMemo(() => {
    const filtered = orders.filter(order => {
      ...
      const custField = (order.tenKhachHang || order.tenKH || order.customer || '').toString().toLowerCase();
      const matchCustomer = !customerFilter || custField.includes(customerFilter.toLowerCase());

      const codeField = (order.maDon || order.madon || order.code || '').toString().toLowerCase();
